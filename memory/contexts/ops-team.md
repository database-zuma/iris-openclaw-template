# OPS Team Context

> **Purpose:** Detailed context for Zuma OPS WhatsApp group interactions
> **Created:** 2026-02-13
> **Status:** Active

---

## People

### Mbak Dini (Bu Dini Tri Mart)
- **Role:** OPS Manager
- **Department:** Operations
- **Context:** Head of OPS team, oversees all operational functions (purchasing, merchandising, allocation, inventory control, branch support)
- **Communication style:** Formal-friendly, Bahasa Indonesia
- **Notes:** Decision maker untuk operational matters

### Mbak Citra
- **Role:** Purchasing Supervisor
- **Department:** Purchasing
- **Context:** Supervises purchasing operations, vendor management, PO coordination
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Handles supplier relationships & procurement

### Mbak Sari
- **Role:** Purchasing Admin
- **Department:** Purchasing
- **Context:** Admin support untuk purchasing — PO processing, documentation, vendor communication
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Executes purchasing transactions & paperwork

### Mas Bagus (Mas Bagus Kiswoyo)
- **Role:** Merchandiser
- **Department:** Merchandising
- **Context:** Product assortment planning, merchandise strategy, stock optimization
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Works closely with Atlas (VPS agent) for stock data & analysis

### Mbak Virra
- **Role:** Allocation Planner
- **Department:** Allocation
- **Context:** Plans product allocation across stores/branches, manages distribution flow
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Uses planogram & RO Request outputs for store replenishment decisions

### Mbak Galuh
- **Role:** Staff Inventory Control (IC)
- **Department:** Inventory Control
- **Context:** Stock monitoring, inventory accuracy, cycle counts, discrepancy resolution
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Works with warehouse & stores on stock accuracy

### Nabila
- **Role:** Staff Inventory Control (IC)
- **Department:** Inventory Control
- **Context:** Stock monitoring, inventory accuracy, cycle counts, discrepancy resolution
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Works with Mbak Galuh on IC tasks

### Mbak Fifi
- **Role:** Branch Support Specialist
- **Department:** Branch Support
- **Context:** Supports branch operations, handles store operational issues, coordination antara HQ & stores
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Bridge between stores & HQ OPS team

### Mbak Nisa
- **Role:** Continuous Improvement Supervisor
- **Department:** Continuous Improvement (CI)
- **Phone:** reference in .env file
- **Context:** Supervises CI initiatives, process improvement projects, operational excellence
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Oversees CI team (Wafi, Wayan), receives daily DB reports

### Wafi
- **Role:** Continuous Improvement - Implementation Specialist
- **Department:** Continuous Improvement (CI)
- **Context:** Implements CI projects, process optimization, operational workflows
- **Communication style:** Professional, Bahasa Indonesia
- **Notes:** Works with Wayan on implementation tasks

### Wayan
- **Role:** Continuous Improvement - System Developer
- **Department:** Continuous Improvement (CI)
- **Context:** System development, automation, data infrastructure, AI/agent setup
- **Communication style:** Casual-professional, Bahasa Indonesia
- **Notes:** Iris's admin/master — setup all OpenClaw agents (Iris, Iris Junior, Atlas, Apollo)

---

## Conversations & Projects

### [Project/Topic Name]
**Date:** [YYYY-MM-DD]
**Participants:** [Who was involved]
**Summary:** [What was discussed, decided, or requested]
**Follow-up:** [Next steps, pending items]

---

## Recurring Requests

### [Type of Request]
**Who asks:** [Name(s)]
**Frequency:** [Daily/weekly/ad-hoc]
**What they need:** [Description]
**How Iris helps:** [Your role/automation]

---

## Struggles & Pain Points

### [Issue/Challenge]
**Who:** [Affected person/team]
**Context:** [What's the problem]
**Iris's role:** [How you help or what to avoid]

---

## Personal Context (Team Members)

### [Name]
- [Personal notes that affect work — e.g., timezone, working hours, preferences]
- [Things they care about, pet peeves, etc.]

---

**Note:** This file auto-grows as group interactions happen. Review monthly to archive old/resolved items.
